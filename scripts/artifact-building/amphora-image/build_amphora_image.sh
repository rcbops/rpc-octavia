...
export BASE_DIR=${PWD}
source /opt/rpc-openstack/scripts/bootstrap-ansible.sh
...
openstack-ansible -i /opt/inventory \
                  ${MY_BASE_DIR}/scripts/artifact-building/amphora-image/rpc-build-image.yml \
...
if [[ "$(echo ${PUSH_TO_MIRROR} | tr [a-z] [A-Z])" == "YES" ]]; then
    if [ -z ${REPO_USER_KEY+x} ] || [ -z ${REPO_USER+x} ] || [ -z ${REPO_HOST+x} ] || [ -z ${REPO_HOST_PUBKEY+x} ]; then
        ...
        openstack-ansible -i /opt/inventory \
                          ${MY_BASE_DIR}/scripts/artifact-building/amphora-image/amphora-image-push-to-mirror.yml \
                          -e rpc_release=${RPC_RELEASE} \
                          -e git_sha=${GIT_SHA} \
                          -e working_dir=${MY_BASE_DIR} \
                          ${ANSIBLE_PARAMETERS}
